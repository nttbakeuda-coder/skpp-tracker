...
// Ajukan KEMBALI pengajuan yang DITOLAK: perbarui data (hasil edit pemohon di
// form) lalu reset status ke 'diajukan' (masuk lagi ke antrean Loket). Pengajuan
// yang SAMA dipakai ulang (id & kodeAkses tetap). `payload` = { nama, nip, opd,
// jabatan, pangkat, alasan } — kolom yg tak dikirim dibiarkan apa adanya.
// RPC SECURITY DEFINER memeriksa kepemilikan + status ditolak.
// return: { data: { ok: true } | null, error }
export async function ajukanUlang(id, payload) {
